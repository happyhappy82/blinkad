...
- ERP 프로젝트/작업관리 아래에 `작업보류매장` 메뉴를 추가했습니다.
  - `언리미티드`, `주도락 강남점`, `주도락 마곡발산점`의 상품 구성과 작업·보고 이력을 작업 보류 상태로 이동했습니다.
  - 작업보류매장 메뉴를 선택하면 왼쪽 메뉴에 세 매장이 하위 목록으로 표시되며, 매장명을 누르면 상세 이력으로 이동합니다.
