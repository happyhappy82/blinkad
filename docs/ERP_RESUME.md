...
- 청구관리 금액 원본을 Notion `매장 마스터` DB에 연결했습니다.
  - 매장명으로 `통합 문의 관리 DB`와 매칭합니다.
  - 월 계약금(VAT 포함), 공급가액, 부가세, 정산금, 월 정산일, 청구 상태, 세금계산서, 원천징수, 계약 상품/일자를 읽습니다.
  - 실제 계약금이 등록된 계약 매장만 청구관리 목록에 표시하며 기존 임시 금액 생성은 제거했습니다.
- 2026-08-26 신규 계약 매장 2곳을 `매장 마스터`에 등록하고 기존 CRM 문의를 `계약 완료`로 전환했습니다.
  - 영종센트럴피부과: VAT 포함 2,000,000원
  - 원스타올드패션드 햄버거 어린이대공원점: VAT 포함 1,100,000원
...
- 청구관리 상태 셀렉터는 아직 화면 로컬 상태입니다. 다음 작업에서 `매장 마스터`의 청구 상태·세금계산서·원천징수·정산 메모 업데이트 API와 연결해야 합니다.
