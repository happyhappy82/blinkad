...
- 후속 월별 연결·제목 갱신을 포함한 테스트는26개 및 tsc 통과. `auto_month_heading=true`로 한국 시간의 월 제목만 갱신하도록 설정했지만 관리DB HTTP404로 실제 자동 쓰기는 여전히 미검증이다. 카드 금액의 현재 월 선택은 네이티브 수식이다. 상세는 `NOTION_MONTHLY_DASHBOARD_20260906.md`.
